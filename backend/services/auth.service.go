...
// returns password reset link, as currently mail service is not used
func (s *authService) RequestPasswordReset(username string) (string, error) {
	user, err := s.userRepo.GetByUsername(username)
	if user == nil || err != nil {
